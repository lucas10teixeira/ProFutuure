    ...
    if (response.ok) {
      ...
      if (dados.success) {
        dados.data.forEach(postagem => {
          ...
          carregarComentarios(postagem.id);
        });
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (dados.success && dados.data) {
          const { nome = 'Usuário desconhecido', sobre = '' } = dados.data;
          
          Swal.fire({
            title: `<strong>${nome}</strong>`,
            html: `<p><strong>Sobre:</strong> ${sobre}</p>`,
            icon: 'info',
            confirmButtonText: 'Fechar'
          });
        } else {
          console.error('Erro ao buscar dados do usuário:', dados.message);
        }
      } else {
