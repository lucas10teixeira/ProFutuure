...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function addPost(post) {
    if (!document.getElementById(`post-${post.id}`)) {
      ...
      let postContent = `
          <p class="post-username" onclick="sobre(${post.userId})"><strong>${post.username}</strong></p>
...
async function sobre(userId) {
  try {
    ...
    const nomeUser = userData.data.nome;
    const sobreUser = userData.data.sobre;

    // Verifica se os dados do usuário foram recebidos com sucesso
    if (userData.success && userData.data) {
      if (userData.data.nome) {
      
        Swal.fire({
          title: `<strong><h2>Usuário:<h2>${nomeUser}</strong>`,
          html: `<p><h2>Sobre:<h2>${sobreUser}</p>`,
          icon: 'info',
